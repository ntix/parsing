...
export class AnyOfValidator<T> implements IParser<T> {
  ...
  readonly parse = parseChain(this.parent, (value) => {
    ...
    if (this.values.some((v) => isEqual(v, value)) === !this.negate) return createParseResult(value);

    const errors = this.negate ? ParseErrors.not(ParseErrors.anyOf(this.values)) : ParseErrors.anyOf(this.values);
